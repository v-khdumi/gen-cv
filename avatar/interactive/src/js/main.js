...
var TTSVoice = "en-US-AvaMultilingualNeural" // Update this value if you want to use different voices
...
supported_languages = ["ro-RO", "en-US", "de-DE", "zh-CN", "nl-NL"] // The language detection engine supports a maximum of 4 languages
          ...
          try {
            ...
            if (responseToken !== undefined && responseToken !== null) {
              try {
                ...
                let product = JSON.parse(responseToken)
...
async function greeting() {
  let text = `Hi, my name is ${TalkingAvatarCharacter}. How can I help you?`
  ...
  avatarSynthesizer.speakSsmlAsync(spokenText, (result) => {
    if (result.reason === SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
      ...
      console.log(
        "Unable to speak text. Result ID: " + result.resultId
      )
...
window.startRecording = () => {
  try {
    ...
    console.log(
      "[INFO] Recording started with speech configuration:",
      speechConfig
    )
  } catch (error) {
...
function makeBackgroundTransparent(timestamp) {
  ...
  if (timestamp - previousAnimationFrameTimestamp > 30) {
    ...
    let tmpCanvasContext = tmpCanvas.getContext("2d", {
